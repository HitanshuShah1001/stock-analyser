...
async function getStockPriceRequest(url: string) {
  try {
    ...
    throw new Error("Unexpected error occured!", e as any);
  }
}

async function getFundamentalsRequest(url: string) {
  try {
...
server.tool(
  "get-stock-data",
  "Get live stock price for a stock",
  {
    ticker: z
      .string()
      .describe(
        "Ticker symbol for a stock - for example for Infosys it will be INFY:NSE"
      ),
  },
...
server.tool(
  "get-income-statement-and-fundamental-data",
  "Get live income statements and fundamentals for a stock",
  {
    ticker: z
      .string()
      .describe(
